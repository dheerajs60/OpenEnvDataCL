...
class Grader:
    ...
    def calculate_final_score(self, orig_df: pd.DataFrame, curr_df: pd.DataFrame) -> float:
        null_score = 1.0 if curr_df.isnull().sum().sum() == 0 else 0.0
        duplicate_score = 1.0 if curr_df.duplicated().sum() == 0 else 0.0
        schema_score = 1.0 if sum(1 for c in curr_df.columns if " " in str(c) or "-" in str(c)) == 0 else 0.0
        date_issues_curr = 0
        ...
        if self.task_difficulty == "easy":
            weights = {"null": 0.60, "duplicate": 0.20, "schema": 0.10, "date": 0.05, "category": 0.05}
